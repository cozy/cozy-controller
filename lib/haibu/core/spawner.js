...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    function onChildStart (monitor, data) {
      ...
      pidFile = path.join(haibu.config.get('directories:pid'), app.name + '.pid');
      if (!fs.existsSync(pidFile)) {
        fs.openSync(pidFile, 'w')
      }
      fs.chmod(pidFile, 0700, function (err) {
        fs.writeFileSync(pidFile, monitor.childData.pid); 
      });
      ...
      result.token = token;
      tokenFile = path.join(haibu.config.get('directories:token'), app.name + '.token');
      if (!fs.existsSync(tokenFile)) {
        fs.openSync(tokenFile, 'w')
      }
      // Change permission
      if (haibu.config.get('useraccounts:prefix')) {
        user = haibu.config.get('useraccounts:prefix') + app.user
        command = 'chown -R ' + user + ':nogroup ' + tokenFile
      ...
      fs.chmod(tokenFile, 0700, function (err) {
