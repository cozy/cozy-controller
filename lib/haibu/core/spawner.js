...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    //
    if (! fs.existsSync(repo.homeDir+'/log')){
      fs.mkdirSync(repo.homeDir+'/log')
    }
    fs.openSync(repo.homeDir+'/log/production.log', 'a')
    logFile = fs.createWriteStream(repo.homeDir + '/log/production.log', {flags:'w'})
    ...
    timeout = setTimeout(onTimeout, haibu.config.get('portTimeout') || 80000);
