...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  // Transmit application's name to drone
  ...
  var server = repo.app.scripts.start;
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    function onChildStart (monitor, data) {
      ...
      //
      // Store application's token in tokenFile
      if (app.name === "home" || app.name === "proxy" || app.name === "data-system") {
        monitor.childData.token = haibu.config.get('authToken');
      } else {
        monitor.childData.token = randomString(40);
      }
      monitor.childData.tokenFile = path.join(haibu.config.get('directories:token'), 
            app.name + '.token');
      if (!fs.existsSync(monitor.childData.tokenFile)) {
        fs.openSync(monitor.childData.tokenFile, 'w')
      }
      // Change permission
      if (haibu.config.get('useraccounts:prefix')) {
        user = haibu.config.get('useraccounts:prefix') + app.user
        command = 'chown -R ' + user + ':nogroup ' + monitor.childData.tokenFile
        exec(command).on('exit', function (exitCode) {
          haibu.emit('tokenFile:chown:exit', 'info', {
             exitCode: exitCode
          })
        });
      }
      fs.chmod(monitor.childData.tokenFile, 0700, function (err) {
        fs.writeFileSync(monitor.childData.tokenFile, monitor.childData.token);
      });
