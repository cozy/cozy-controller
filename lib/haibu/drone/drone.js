...
/*Drone.prototype.lightUpdate = function (app, callback) {
  ...
  appDir = drone.drones[0].spawnWith.cwd;
  if (app.repository.type === "git") {
    process.chdir(appDir);
    //git pull
    haibu.repository.git.update( function(err) {
      if (err) {
        callback(err);
      } else {
        // brunch build
        haibu.common.brunch.build(appDir, function (isBuild, err) {
          return callback(isBuild, err);
        });
      };
    });
  } else {
...
Drone.prototype.buildBrunch = function(appName, callback) {
  callback = callback || function () {};
  drone = this.show(appName);
  haibu.common.brunch.build(drone.drones[0].spawnWith.cwd, function (isBuild, err) {
    return callback(isBuild, err);
  });
}