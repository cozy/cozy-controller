...
exports.autostart = function (server, callback) {
  ...
  function orderDrones (files, callback) {
    file = files.pop()
    if (file == undefined) {
      if (dataSystem != "") {
        orderedFiles = [dataSystem].concat(orderedFiles);
      }
      if (home != "") {
        orderedFiles.push(home)
      }
      ...
      fs.readFile(path.join(autostartDir, file), function (err, pkg) {
        ...
        if (pkg.name == 'home') {
          home = file;
          orderDrones (files, callback);
        } else if (pkg.name == 'data-system') {
          dataSystem = file;
          orderDrones (files, callback);
        } else {
  ...
  fs.readdir(autostartDir, function (err, files) {
    ...
    orderDrones(files, function (err, files) {
      async.map(files, function (file, next) {
        ...
        fs.readFile(path.join(autostartDir, file), function (err, pkg) {
          ...
          if (pkg.name == "proxy") {
            setTimeout(function(){startDrones(pkg, next)}, 200000);
          } else {
