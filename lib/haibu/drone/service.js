...
exports.createRouter = function (drone) {
  ...
  haibu.router.path('/drones', function () {
    ...
    this.post('/:id/lightUpdate', function (id) {
      var res = this.res;
      drone.lightUpdate(this.req.body.update.name, function (isBuild, err) {
        if (err) {
          haibu.emit('error:service', 'error', err);
          return haibu.sendResponse(res, 500, { error: err });
        } else {
          return haibu.sendResponse(res, 200, { update: isBuild });
        }
      });
    ...
    this.post('/:id/brunch', function (id) {
      var res = this.res;
      drone.buildBrunch(this.req.body.brunch.name, function (isBuild, err) {
        if (isBuild) {
          if (err) {
            haibu.emit('error:service', 'error', err);
            return haibu.sendResponse(res, 500, { error: err });
          }
          haibu.sendResponse(res, 200, { brunch: isBuild });
        } else {
