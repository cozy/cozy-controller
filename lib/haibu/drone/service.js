...
exports.createRouter = function (drone) {
  ...
  haibu.router.path('/drones', function () {
    ...
    //
    // ### Update Drone for App
    // `POST /drones/:id/lightUpdate` pull changes in
    // the app with :id on this server.
    //
    this.post('/:id/lightUpdate', function (id) {
      var data = drone.show(id);
      var res = this.res;
      if (typeof data === 'undefined') {
        haibu.sendResponse(this.res, 404, { message: 'No drone(s) found for application ' + id });
        ...
        drone.lightUpdate(this.req.body, function (err) {
          if (err != undefined) {
            haibu.emit('error:service', 'error', err);
            return haibu.sendResponse(res, 500, { error: err });
          }

          haibu.sendResponse(res, 200, { lightUpdate: true });
        });
      };
    });
    ...
    this.post('/:id/brunch', function (id) {
      ...
      if (typeof data === 'undefined') {
        ...
        drone.buildBrunch( data.drones[0].spawnWith.cwd, function (isBuild, err) {
          if (isBuild) {
            if (err) {
              haibu.emit('error:service', 'error', err);
              return haibu.sendResponse(res, 500, { error: err });
            }
            haibu.sendResponse(res, 200, { brunch: true });
          } else {
            haibu.sendResponse(res, 200, { brunch: false });
          };
        });
      };
    });
