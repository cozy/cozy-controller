...
exports.createRouter = function (drone) {
  ...
  // POST /authToken
  //
  haibu.router.post('/authToken', function() {
    var res = this.res;
    token = haibu.config.get('authToken');
    //
    // Home lives on port 9103
    //
    data = {method: "POST",uri: "http://localhost:9103/haibu/token", json: {password: token}};
    request(data, function(err, res, body) {
    });
    haibu.sendResponse(res, 200, {token: true});
  })
  ...
  haibu.router.path('/drones', function () {
    ...
    this.post('/:id/light-update', function (id) {
      ...
      drone.lightUpdate(this.req.body.update, function (isBuild, err) {
        if (err) {
          haibu.emit('error:service', 'error', err);
          return haibu.sendResponse(res, 500, { error: err });
        } else {
