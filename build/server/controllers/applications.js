...
var async, controller, exec, log, restartController, sendError, updateController, updateMonitor;
...
updateMonitor = function(count, callback) {
  log.info("monitor: update");
  return exec("npm -g update cozy-monitor", function(err, stdout, stderr) {
    if (err || stderr) {
      if (count < 2) {
        return updateMonitor(count + 1, callback);
      } else {
        return callback("Error during monitor update after " + (count + 1) + " try: " + stderr);
      }
    } else {
      return callback();
    }
